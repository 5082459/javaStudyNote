...
**submit() **：提交**需要返回的任务**，返回Future类型的对象。使用**get()方法会阻塞当前线程直到任务完成**，而使用get（long timeout，TimeUnit unit）方法则会**阻塞当前线程一段时间后立即返回**，这时候有可能任务没有执行完。
...
HotSpot VM的线程模型：
- java线程一一映射为本地操作系统线程，线程终止，系统线程会被回收
两级调度模式：
- 上层：java多线程程序将应用**分为若干个任务**，使用户级的调度器（**Executor框架**）将这些任务**映射**成**固定数量的线程**。（executor框架控制）
- 底层：**操作系统内核**将这些**线程映射到硬件处理器上**。（操作系统内核控制）
![](images/2019-10-17-20-59-43.png)

1. **Executor框架的结构**
- 任务：包括被执行任务需要的接口：Runnable接口或Callable接口。
- 任务的执行：核心接口Executor，两个关键类（ThreadPoolExecutor和ScheduledThreadPoolExecutor）
...
类或接口|描述
:-:|:-:
Executor|接口，将任务的提交和执行分开
ThreadPoolExecutor|线程池核心实现类，用来执行被提交的任务
ScheduledThreadPoolExecutor|实现类，可以延时或定期执行命令
Future|接口，实现类为FutureTask，代表异步计算结果
Runnable接口和Callable接口的实现类|都可以被ThreadPoolExecutor或ScheduledThreadPoolExecutor执行。
![](images/2019-10-17-21-08-49.png)
![](images/2019-10-17-21-10-01.png)

1. 主线程创建**实现Runnable或Callable接口的任务**
2. 将Runnable对象交给ExecutorService执行，行（**ExecutorService.execute（Runnablecommand）**）；或者也可以把Runnable对象或Callable对象提交给ExecutorService执行（**ExecutorService.submit（Runnable task）**或**ExecutorService.submit（Callable<T>task）**）。
3. 行ExecutorService.submit（…），将返回Funture接口（FutureTask）的对象。
...
类或接口|描述
:-:|:-:
ThreadPoolExecutor|通常使用工厂类Executor来创建。可创建：ThreadPoolExecutor SingleThreadExecutor、FixedThreadPool和CachedThreadPool。
FixedThreadPool|创建使用固定线程数的API，适合用于负载比较重点的服务器
SingleThreadExecutor|创建单个线程，适合用于保证顺序执行各项任务
CachedThreadPool|根据需要创建线程，是一个大小无界的线程池
ScheduledThreadPoolExecutor|通常使用工厂类Executors来创建。Executors可以创建2种类型的ScheduledThreadPoolExecutor|适用于单个后台执行周期任务
ScheduledThreadPoolExecutor|包含若干个线程的ScheduledThreadPoolExecutor。
SingleThreadScheduledExecutor|只包含一个线程的ScheduledThreadPoolExecutor。适用于保证任务顺序执行的得场景
Future|实现类FutureTask用来表示异步计算结果的。
Runnable|任务，不返回结果
Callable|任务，返回结果

### 10.2 ThreadPoolExecutor详解

ThreadPoolExecutor是Executor框架最核心的实现类。
- corePool：核心线程的大小
- maximumPool：最大线程数
...
![](images/2019-10-17-21-34-08.png)
...
![](images/2019-10-17-21-36-16.png)
**底层调用ThreadPoolExecutor**
...
![](images/2019-10-17-21-38-20.png)
...
1. scheduleAtFixedRate()或者scheduleWithFixedDelay()：会向ScheduledThreadPoolExecutor的**DelayQueue**添加一个实现了RunnableScheduledFutur接口的ScheduledFutureTask。
...
![](images/2019-10-17-21-44-58.png)
...
## 10.4 FutureTask详解
**10.4.1 FutureTask简介**
FutureTask除了实现Future接口外，还实现了Runnable接口因此，FutureTask可以交给Executor执行，也可以由调用线程直接执行（FutureTask.run()）。
...
总结：
![](images/2019-10-17-21-50-12.png)
...
**11.1.1 生产者消费者模式实战**
**11.1.2 多生产者和多消费者场景**
![](images/2019-10-17-22-07-21.png)
**11.1.3 线程池与生产消费者模式**
**IO密集型任务**
**CPU密集型任务**